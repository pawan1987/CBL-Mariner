# Hack: the toolkit does not correctly include BuildRequires so we need to manually define the systemd-rpm-macros we use
%if %{undefined _sysusersdir}
%define _sysusersdir /usr/lib/sysusers.d
%endif
%if %{undefined _tmpfilesdir}
...
Source0: https://pagure.io/setup/archive/%{name}-%{version}/%{name}-%{name}-%{version}.tar.gz#/%{name}-%{version}.tar.gz
BuildArch: noarch
...
%changelog
* Tue Dec 12 12:28:58 EST 2023 Dan Streetman <ddstreet@ieee.org> - 2.14.5-2
- Initial import to Azure Linux
